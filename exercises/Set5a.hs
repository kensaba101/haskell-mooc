...
totalPrice (MkShoppingEntry _ a b) = a*(fromIntegral b) 
...
buyOneMore (MkShoppingEntry x y b) = MkShoppingEntry x y (b+1)
...
--{ name :: String, age :: Int, town :: String, state :: String, profession :: String}
...
data Position = PositionDefined {x :: Int, y :: Int}

-- origin is a Position value with x and y set to 0
origin :: Position
origin = PositionDefined 0 0 

-- getX returns the x of a Position
getX :: Position -> Int
getX p = x p
-- if you didn't use the record syntax:
--getX (PositionDefined x y) = x

-- getY returns the y of a position
getY :: Position -> Int
getY p =  y p 
--getY (PositionDefined x y) = y

-- up increases the y value of a position by one
up :: Position -> Position
up p = PositionDefined (getX p) ((getY p)+1)

-- right increases the x value of a position by one
right :: Position -> Position
right p = PositionDefined ((getX p) +1) (getY p)
...
data UpDown = UpDownInc Int | UpDownDec Int
...
tick :: UpDown -> UpDown
tick (UpDownInc x) = UpDownInc (x+1)
tick (UpDownDec x) = UpDownDec (x-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpDownInc x) = UpDownDec x
toggle (UpDownDec x) = UpDownInc x
...
rgb (Mix col1 col2) = zipWith (\x a -> ((x+a)/2)) (rgb col1) (rgb col2)
------------------------------------------------------------------------------
...
toBin 0 = (O End)
toBin 1 = (I End)
toBin x | (x `mod` 2) == 0 = O (toBin ((div) x 2))
toBin x | (x `mod` 2) /= 0 = I (toBin ((div) x 2))

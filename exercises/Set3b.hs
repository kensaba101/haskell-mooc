...
sums' prevSum toAdd max | toAdd == max = [prevSum + toAdd]
sums' prevSum toAdd max = (prevSum + toAdd):(sums' (prevSum+toAdd) (toAdd+1) max)
...
sumsOf' [] _ = []
sumsOf' (x:xs) prev = (x+prev):(sumsOf' xs (x+prev))

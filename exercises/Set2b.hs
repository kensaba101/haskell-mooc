...
countdown x = "Ready! " ++ counting x
 where counting 1 = "1... Liftoff!"
       counting x = show(x) ++ "... " ++ counting (x-1)
...
smallestDivisor n = testdiv n 2 
 where testdiv n divisor | ((mod n divisor) == 0) = divisor
       testdiv n divisor = testdiv n (divisor + 1)

------------------------------------------------------------------------------
-- Ex 7: implement a function isPrime that checks if the given number
-- is a prime number. Use the function smallestDivisor.sss
--
...
biggestPrimeAtMost x | (isPrime x) = x
biggestPrimeAtMost x = biggestPrimeAtMost (x-1)

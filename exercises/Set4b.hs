...
concatStart = []
concatHelper ys result = ys ++ result
...
largestHelper y []= [y]
largestHelper y ys | y == head ys = (y:ys)
largestHelper y ys | y < head ys = ys
largestHelper y ys | y > head ys = [y]
